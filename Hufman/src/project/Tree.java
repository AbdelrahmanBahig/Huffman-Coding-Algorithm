...
public class Tree {
    ...
    public void getNode(char search , Node root)
    {
        if(appeared.contains(search))
        {
            if(root != null)
            {
                if(root.symbol == search) temp = root;
...
//----------------------------------------------------------------------------------------------------------------------
    public void updateTree(char symbol , Node node , Node root , boolean CD)
    {
        ...
        if(!appeared.contains(symbol))
        {
            ...
            if(CD)Result += node.code + sc.get(index);
    ...
    public void swapH(Node root)
    {
        ...
        if(root.left.symbol == '*') swapH(root.left);
        else if (root.right.symbol == '*') swapH(root.right);
    }
        ...
        for(int i=0 ; i<nodes.size() ; i++)
        {
            for(int j=0 ; j<nodes.size() ; j++)
            {
                if(i != j) {
                    ...
                    {
                        //Check Right or Left
                        if(nodes.get(i) == nodes.get(i).parent.right) firstRight = true;
                        if(nodes.get(j) == nodes.get(j).parent.right) secondRight = true;
    ...
    public char check(String code , Node root)
    {
        ...
        {
            if(code.charAt(i) == '0') root = root.left;
            else if(code.charAt(i) == '1') root = root.right;
        }
        return root.symbol;
    }
    public void handleCode(Node current)
    {
        ...
        if(current.left.symbol == '*') handleCode(current.left);
        else if(current.right.symbol == '*') handleCode(current.right);
    }
//----------------------------------------------------------------------------------------------------------------------
    public void print()
    {
